...
import { CloseMergeRequestParameters, CloseMergeRequestActionResult, CloseMergeRequestError } from "@app/models";
...
/** Props type of `CloseMergeRequestResultDisplay` */
type CloseMergeRequestResultDisplayProps = ActionResultDisplayProps<CloseMergeRequestActionResult, CloseMergeRequestParameters>;

/** CloseMergeRequest operation result display component */
const CloseMergeRequestResultDisplay: FC<CloseMergeRequestResultDisplayProps> = ({
    result, parameters, projects
}) => {
    const errors = useMemo(() => result.errors.groupBy<CloseMergeRequestError>("type"), [result.errors]);
    ...
    return (
        <section role="close-merge-request-results">
            ...
            <section>
                <Accordion
                    defaultExpanded
                    caption={`${getLocalizedText("results.mergeRequest.close.successCaption")} (${result.closed.length})`}
                >
                    {result.closed.length === 0
                        ? <span className="has-text-grey has-text-wrapped has-text-centered">
                            {getLocalizedText("results.mergeRequest.close.noSuccessMessage")}
                        </span>
                        : <ul>
                            {result.closed.map(x =>
                                <ResultListItem
                                    ...
                                    text={getLocalizedText("results.mergeRequest.close.requestClosed")}
                                />
                            )}
                        </ul>
...
export default CloseMergeRequestResultDisplay;
